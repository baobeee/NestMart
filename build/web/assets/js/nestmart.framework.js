...
;(function ($) {
    ...
    let $clone_to_mobile_sidebar 	= $('.clone-to-mobile-sidebar'),
    	...
    	nestmart_MOBILE_MENU 		= {
    		...
    		clone_menus: function (obj) {
				...
				obj.each(function () {
				    let $this               = $(this),
				        ...
				        $this_menu_clone_id = 'nestmart-clone-' + $this_menu_id;
				    if (!$('#' + $this_menu_clone_id).length) {
				        ...
				        $thisClone.find('[id]').each(function () {
				            $__this.attr('id', nestmart_MOBILE_MENU.add_string_prefix( this_id, 'nestmart-clone-'));
				        });
				        $thisClone.find('.nestmart-menu').addClass('nestmart-menu-clone');
				        let $thisMainPanel = $('#nestmart-clone-wrap .nestmart-panels #nestmart-main-panel ul');
				        $thisMainPanel.append( $thisClone.html());
				        nestmart_MOBILE_MENU.insert_children( $thisMainPanel, i);
				    }
				});

				$document.on('click', '.nestmart-next-panel', function (e) {
		            e.preventDefault();
		            let $this     = $(this),
		                thisItem  = $this.closest('.menu-item'),
		                thisPanel = $this.closest('.nestmart-panel'),
		                target_id = $this.attr('href');
		            if ($(target_id).length) {
		                thisPanel.addClass('nestmart-sub-opened');
		                $(target_id).addClass('nestmart-panel-opened').removeClass('nestmart-hidden').attr('data-parent-panel', thisPanel.attr('id'));
		                /*Insert current panel title*/
		                let item_title          = thisItem.find('.menu-name').attr('data-title'),
		                    firstItemTitle      = '',
		                    $nestmart_panels     = $('.nestmart-panels-actions-wrap'),
		                    current_panel_title = $nestmart_panels.find('.nestmart-current-panel-title');
		                ...
		                if ( typeof item_title !== 'undefined') {
		                    if (current_panel_title.length) {
		                        ...
		                        $nestmart_panels.prepend('<span class="nestmart-current-panel-title">'+item_title+'</span>');
		                    }
		                }else {
		                    current_panel_title.remove();
		                }
		                $nestmart_panels.find('.nestmart-prev-panel').remove();
		                $nestmart_panels.prepend('<a data-prenttitle="'+firstItemTitle+'" class="nestmart-prev-panel" href="#' + thisPanel.attr('id') + '" data-cur-panel="' + target_id + '" data-target="#' + thisPanel.attr('id') + '"></a>');
		            }
		        });

		        $document.on('click', '.nestmart-prev-panel', function (e) {
		            e.preventDefault();
		            let $this                   = $(this),
		                cur_panel_id            = $this.attr('data-cur-panel'),
		                target_id               = $this.attr('href'),
		                current_panel_title     = $('.nestmart-panels-actions-wrap .nestmart-current-panel-title'),
		                previous_panel          = $('.nestmart-panels-actions-wrap .nestmart-prev-panel');

		            $(cur_panel_id).removeClass('nestmart-panel-opened').addClass('nestmart-hidden');
		            $(target_id).addClass('nestmart-panel-opened').removeClass('nestmart-sub-opened');

		            /*Set new back button*/
		            let new_parent_panel_id = $(target_id).attr('data-parent-panel');
		            if ( typeof new_parent_panel_id == 'undefined') {
		                let root_title = $('.nestmart-panels-actions-wrap').data('root_title');
		                ...
		                let item_title = $('#' + new_parent_panel_id).find('.nestmart-next-panel[data-target="' + target_id + '"]').closest('.menu-item').find('.menu-name').attr('data-title');
		                if ( typeof item_title !== 'undefined') {
		                    if ( !current_panel_title.length) {
		                        $('.nestmart-panels-actions-wrap').prepend('<span class="nestmart-current-panel-title">'+item_title+'</span>');
		                    }else{
		                        current_panel_title.html(item_title);
		                    }
		                }else {
		        ...
		        setTimeout(function () {
		            $('#nestmart-clone-wrap .nestmart-panels').find('.nestmart-carousel').nestmart_init_carousel();
		        }, 1000);
			},
		        ...
		        if ( children.length) {
		            children.each(function () {
		                let thisChildItem   = $(this),
		                	next_nav_target = 'nestmart-panel-' + String(index);
		                nestmart_MOBILE_MENU.insert_children(thisChildItem,index);
		                while ($('#' + next_nav_target).length) {
		                    index++;
		                    next_nav_target = 'nestmart-panel-' + String(index);
		                }
		                thisChildItem.prepend('<a class="nestmart-next-panel" href="#' + next_nav_target + '" data-target="#' + next_nav_target + '"></a>');
		                ...
		                $('#nestmart-clone-wrap .nestmart-panels').append('<div id="' + next_nav_target + '" class="nestmart-panel nestmart-sub-panel nestmart-hidden">' + submenu_html + '</div>');
		            });
		        }
		    },
		    ...
		    wrap_container: function(){
		    	if($body.find('#nestmart-clone-wrap').length) return 0;
				let mb_menu	='<div id="nestmart-clone-wrap" class="nestmart-clone-wrap">';
				mb_menu +='<div class="nestmart-panels-actions-wrap" data-root_title="Main Panel">';
				mb_menu +='<span class="nestmart-current-panel-title">Main Panel</span>';
				mb_menu +='<a class="nestmart-close-btn nestmart-close-panels" href="#" data-object="open-mobile-menu">&times;</a>';
				mb_menu +='</div>';
				mb_menu +='<div class="nestmart-panels">';
				mb_menu +='<div id="nestmart-main-panel" class="nestmart-panel nestmart-main-panel">';
    ...
    $.fn.nestmart_init_carousel = function () {

        if( typeof $.fn.slick == 'undefined'){
            console.error('nestmart-Framework need to use slick library inside, pls make sure It was loaded before used');
            return;
        }
        ...
        $(this).not('.slick-initialized').each(function () {
            ...
            if (config.vertical === true ) {
                config.prevArrow = '<span class="nestmart-icon icon-left-arrow prev"></span>';
                config.nextArrow = '<span class="nestmart-icon icon-arrow-right next"></span>';
            } else {
                config.prevArrow = '<span class="nestmart-icon icon-left-arrow prev"></span>';
                config.nextArrow = '<span class="nestmart-icon icon-arrow-right next"></span>';
            }
            ...
            slide.on('init', function (event, slick) {
                $(event.target).trigger( 'nestmart_trigger_init_slide', slick);
            });
            slide.slick(config);
        });
    };

    $.fn.nestmart_vertical_menu = function () {
        $(this).each( function () {
            let this_menu = $(this);
    ...
    $.fn.nestmart_sticky_header = function () {
        $(this).each(function () {
            let _this           = $(this),
            ...
            TAB.on('click','.tab-link', function (e) {
                ...
                if( !$tab_head.hasClass('active') && typeof tab_id !== undefined){
                    ...
                    if( $active_tab.length ){
                        ...
                        $active_tab.addClass('active').nestmart_equal_height();
    ...
    $.fn.nestmart_rating_form_handle = function () {
        $(this).each(function(index,element){
    ...
    $.fn.nestmart_accodition_handle = function () {
        $(this).each(function(index,element){
    ...
    $.fn.nestmart_countdown = function () {
        if( typeof $.fn.countdown == 'undefined'){
            console.error('nestmart-Framework need to use countdown library inside, pls make sure It was loaded before used');
            return;
        }
        let $this           = $(this),
            html_result     = '',
        ...
        $this.on('nestmart_countdown', function () {
            $this.each(function () {
                let $element       = $(this),
        ...
        }).trigger('nestmart_countdown');
    };

    $.fn.nestmart_equal_height  = function () {
    ...
    $.fn.nestmart_stretch_the_right_background = function () {
    ...
    $.fn.nestmart_menu_mobile = function () {
        nestmart_MOBILE_MENU.init(this);
    };

    $.fn.nestmart_sidebar_handle = function(){
        if( $clone_to_mobile_sidebar.length){
        	if( $sidebar.length ){
	            ...
	            html_sidebar +='<div class="nestmart-mobile-panels">';
	            html_sidebar +='<span class="nestmart-current-panel-title">Sidebar</span>';
	            html_sidebar +='<a class="nestmart-close-btn" href="#" data-object="open-mobile-filter">&times;</a>';
	            html_sidebar +='</div>';
	            ...
	            if( $main_content.length){
	                ...
	                let $caroulsel_inside = $sidebar_content.find('.nestmart-carousel');
	                if( $caroulsel_inside.length){
	                    $caroulsel_inside.nestmart_init_carousel();
	                }
	            }
	        }
	    }
